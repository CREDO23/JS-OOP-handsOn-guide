...
- The object 'admin'  was created with the following properties: ${ps(`[${Object.keys(admin).join(', ')}]`)} ${s2}`)
...
- The object 'user' was created without properties: ${ps(`[${Object.keys(user)}]`)} ${s2}`)
...
console.log(` ${lns2(`Line ${ln()} : `)} ${s1}
- Now the object 'user' has the following properties: ${ps(`[${Object.keys(user).join(', ')}]`)} ${s2}`)
...
console.log(` ${lns2(`Line ${ln()} : `)} ${s1}
- So far, the object 'admin' has the property 'age' set to ${ps(admin.age)} ${s2}`)

admin.age = 22

console.log(` ${lns2(`Line ${ln()} : `)} ${s1}
- Now the object 'admin' has the property 'age' set to ${ps(admin.age)} ${s4}`)
...
(user.name) -----> ${ps(user.name)} ${s2}`)

delete user.name
delete admin.age

console.log(` ${lns2(` At line [${ln() - 3}] `)}, the property name of the object 'user' has been deleted,
 so if we try to access it, we will get 'undifined' as result:  (user.name) -----> ${ns(user.name)} ${s4}`)
...
- We created the 'admin' object with a multi word property ['is active']; so far
  ...
  multi word property [e.g: admin.is active], it will result in a ${ns('syntax error' )} !
...
- We created the 'studentsScore' object with two properties as result of a concatenation
   ...
   (studentsScore[student1] or studentsScore["John" + "Smith"]) -----> ${ps(studentsScore[student1])}
...
- Now we can get or set that property: (car.owner) -----> ${ps(car.owner)} ${s4}`)
...
- Some properties have been deleted from the 'admin' object, we can now check by using the ['in'] operator:
  ('age' in admin) -----> ${ns("age" in admin)} or ('id' in admin) -----> ${ps("id" in admin)} ${s4}`)
